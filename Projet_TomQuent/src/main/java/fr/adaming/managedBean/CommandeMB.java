...
public class CommandeMB implements Serializable {
	...
	// Setters pour l'injection de d√©pendance des @ManagedProperty
	...
	public void startCommande() {
		...
		if (this.viewCommande != false) {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_WARN, "Impossible !", "Une commande est dÈj‡† en cours"));
		} else {
	...
	public String passerCommande() {
		...
		// D√©marrage d'une commande et ajout d'une commande
		Date date = new Date();
		date.getTime();
		...
		// v√©rification de la pr√©sence d'un client en session
		if (this.commande.getClient().getId() != 0) {
			...
			if (this.commande.getIdCom() != 0) {
				...
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, "La commande a ÈchouÈ!", ""));
				return "panier";
			}
			...
			String msg = "Bonjour\n\n Une nouvelle commande ‡ traiter a ÈtÈ passÈe, voir la PJ pour le dÈtail.";

			mailsender.sendMail("michalbebert@gmail.com", "Nouvelle Commande", msg, this.commande);

			// R√©initialiser la vue de l'accueilCommande + panier
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "La commande enregistrÈe !", ""));

			return "recapitulatifCommande";
		} else {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"La commande a ÈchouÈ!", "Vous devez vous connecter pour passer commande"));
			return "panier";
		}
	...
	public String modifCommande() {
		...
		if (verif != 0) {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage("La modification de la commande a ÈchouÈ"));
			return "modificationCommande";
		}
	}

	// Supprimer une commande // Faire que le client ne peut supprimer sa commande
	// que 1j aprËs la commande ?!
	...
	public String supprCommandeClient() {
		...
		if (dateAjd - this.commande.getDate().getTime() <= 86400000) {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Impossible", "Supprimer la commande n'est pas possible, votre dÈlai est dÈpassÈe"));
			return "espaceClient";
		}

	}

	// GetAllCommande pas √† faire ??! et GetOneCOmmande non plus..? Surtout si on a
	// un <select> dans le tableau ?

	// Generer le pdf de r√©capitulatif de la commande
	...
	public void detailsCommandePdf(Commande commande, List<LigneCommande> listeLignesCommandes) {
		...
		try {
			...
			paragraph.add("RÈcapitulatif de la commande du " + commande.getDate() + "\n de M/Mme "
			...
			for (int i = 0; i < listeLignesCommandes.size(); i++) {
				...
				tableListe.addCell(cell);
				cell = new PdfPCell(
						new Phrase((String.valueOf(listeLignesCommandes.get(i).getProduit().getPrix() + "Ä"))));
				tableListe.addCell(cell);
				...
				cell = new PdfPCell(new Phrase(String.valueOf(montantLigne) + "Ä"));
				tableListe.addCell(cell);
			}
			...
			cell = new PdfPCell(new Phrase(String.valueOf(montantTotal) + "Ä"));
			cell.setColspan(2);
			tableListe.addCell(cell);
